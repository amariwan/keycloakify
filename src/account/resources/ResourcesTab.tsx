 ...
 *
 * $ npx keycloakify own --path 'account/resources/ResourcesTab.tsx'
 *
...
import { ContinueCancelModal, useEnvironment } from "../../shared/keycloak-ui-shared";
import {
  Button,
  Chip,
  ChipGroup,
...
export const ResourcesTab = ({ isShared = false }: ResourcesTabProps) => {
  ...
  const toggleOpen = async (id: string, field: keyof PermissionDetail, open: boolean) => {
  ...
  return (
    <>
      <ResourceToolbar
        ...
        onPerPageSelect={(first, max) => setParams({ first: `${first}`, max: `${max}` })}
        hasNext={!!links?.next}
      />
      <Table aria-label={t("resources")}>
        <Thead>
          <Tr>
            ...
            <Th>{t("application")}</Th>
            <Th aria-hidden={isShared}>{!isShared ? t("permissionRequests") : ""}</Th>
          </Tr>
        </Thead>
        {resources.map((resource, index) => (
          <Tbody key={resource.name} isExpanded={details[resource._id]?.rowOpen}>
            <Tr>
              <Td
              ...
              />
              <Td dataLabel={t("resourceName")} data-testid={`row[${index}].name`}>
                {resource.name}
              </Td>
              ...
              <Td dataLabel={t("permissionRequests")}>
                {resource.shareRequests && resource.shareRequests.length > 0 && (
                  <PermissionRequest resource={resource} refresh={() => refresh()} />
                )}
                <ShareTheResource
                  resource={resource}
                      ...
                      >
                        <DropdownList>
                          ...
                          <OverflowMenuDropdownItem
                            ...
                            isDisabled={details[resource._id]?.permissions?.length === 0}
                          >
                            <EditAltIcon /> {t("edit")}
                          </OverflowMenuDropdownItem>
                          <ContinueCancelModal
