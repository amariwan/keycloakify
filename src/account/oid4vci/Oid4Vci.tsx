 ...
 *
 * $ npx keycloakify own --path 'account/oid4vci/Oid4Vci.tsx'
 *
...
export const Oid4Vci = () => {
  ...
  useEffect(() => {
    if (initialSelected !== selected && credentialsIssuer !== undefined) {
      requestVCOffer(context, selectOptions[selected], credentialsIssuer).then(blob => {
            ...
            >
              <SelectList>
                {dropdownItems.map(option => (
                  <SelectOption key={option} value={option} data-testid={option}>
                    {option}
                  </SelectOption>
                ))}
              </SelectList>
