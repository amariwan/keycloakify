...
import { FormSubmitButton, TextControl, useFetch } from "../../shared/keycloak-ui-shared";
  ...
  const duplicateGroup = async (
    ...
    try {
      const newGroup: GroupRepresentation = {
        ...sourceGroup,
        name: isSubGroup ? sourceGroup.name : t("copyOf", { name: sourceGroup.name }),
      ...
      if (permissions) {
        await adminClient.groups.updatePermission({ id: createdGroup.id }, permissions);
      }
  ...
  const submitForm = async (group: GroupRepresentation) => {
    ...
    try {
      if (duplicateId && duplicateGroupDetails) {
        ...
        await adminClient.groups.update({ id }, { ...rename, name: group.name });
      } else {
      ...
      handleModalToggle();
      addAlert(
        t(rename ? "groupUpdated" : duplicateId ? "groupDuplicated" : "groupCreated"),
