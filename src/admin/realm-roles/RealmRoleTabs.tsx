...
export default function RealmRoleTabs() {
  ...
  const useTab = (tab: RealmRoleTab | ClientRoleTab) => useRoutableTab(toTab(tab));
      ...
      <PageSection variant="light" className="pf-v5-u-p-0">
        <FormProvider {...form}>
          <RoutableTabs isBox mountOnEnter defaultLocation={toTab("details")}>
            <Tab title={<TabTitleText>{t("details")}</TabTitleText>} {...detailsTab}>
              <RoleForm
                form={form}
                onSubmit={onSubmit}
            ...
            {!isDefaultRole(roleName) && (
              ...
              >
                <AttributesForm
                  ...
                  reset={() => setValue("attributes", attributes, { shouldDirty: false })}
                />
              </Tab>
            )}
            {!isDefaultRole(roleName) && (
            ...
            {isFeatureEnabled(Feature.AdminFineGrainedAuthz) && canViewPermissionsTab && (
              <Tab
