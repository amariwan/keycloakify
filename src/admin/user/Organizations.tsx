...
export const Organizations = ({ user }: OrganizationProps) => {
  ...
  const onSelect = (_event: any, value: string) => {
    if (filteredMembershipTypes.includes(value)) {
      setFilteredMembershipTypes(filteredMembershipTypes.filter(item => item !== value));
    } else {
  ...
  return (
    <>
        ...
        <OrganizationModal
          ...
          onAdd={async orgs => {
            try {
              ...
              addAlert(
                t(shouldJoin ? "userAddedOrganization" : "userInvitedOrganization", {
                  count: orgs.length
                })
              );
              refresh();
      ...
      <OrganizationTable
        ...
        loader={userOrgs}
        isSearching={searchTriggerText.length > 0 || filteredMembershipTypes.length > 0}
          ...
          <>
            ...
            <ToolbarItem>
              <Dropdown
                ...
                toggle={ref => (
                  <MenuToggle ref={ref} id="toggle-id" onClick={toggle} variant="primary">
                    {t("joinOrganization")}
                  </MenuToggle>
                )}
                isOpen={joinToggle}
