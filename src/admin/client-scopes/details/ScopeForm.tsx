      ...
      <FormProvider {...form}>
        <TextControl
          ...
          rules={{
            ...
            onChange: e => {
              if (isDynamicScopesEnabled) setDynamicRegex(e.target.validated, true);
            }
          }}
        />
        ...
        />
        <ActionGroup>
          <FormSubmitButton formState={formState} disabled={!isDirty || !isValid}>
            {t("save")}
          </FormSubmitButton>
          <Button
            variant="link"
            component={props => <Link {...props} to={toClientScopes({ realm })}></Link>}
          >
            {t("cancel")}
          </Button>
        </ActionGroup>
