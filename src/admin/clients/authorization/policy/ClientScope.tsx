...
export const ClientScope = () => {
  ...
  return (
    <FormGroup
      ...
      labelIcon={
        <HelpItem helpText={t("clientsClientScopesHelp")} fieldLabelId="clientScopes" />
      }
    ...
    >
      <Controller
        ...
        defaultValue={[]}
        rules={{
          validate: (value: RequiredIdValue[]) => value.filter(c => c.id).length > 0
        }}
        render={({ field }) => (
          <>
            {open && (
              <AddScopeDialog
                clientScopes={scopes.filter(
                  scope =>
                    !field.value.map((c: RequiredIdValue) => c.id).includes(scope.id!)
                )}
                isClientScopesConditionType
                open={open}
                toggleDialog={() => setOpen(!open)}
                onAdd={scopes => {
                  setSelectedScopes([...selectedScopes, ...scopes.map(s => s.scope)]);
                  field.onChange([
                    ...field.value,
                    ...scopes
      ...
      {errors.clientScopes && <FormErrorText message={t("requiredClientScope")} />}
    </FormGroup>
  );
};
