...
export const ResourcesPolicySelect = ({
  ...
  const toChipGroupItems = (field: ControllerRenderProps<PolicyRepresentation, Type>) => {
    return (
      <ChipGroup>
        {field.value?.map(permissionId => {
          const item = items.find(permission => permission.id === permissionId);
    ...
    <>
      ...
      {createPolicyDialog && (
        <NewPolicyDialog
          ...
          onSelect={p => {
            navigate(toCreatePolicy({ id: clientId, realm, policyType: p.type! }));
          }}
