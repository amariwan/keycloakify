...
export default function ImportForm() {
  ...
  async function parseFileContents(contents: string): Promise<ClientRepresentation> {
    ...
    if (!response.ok) {
      throw new Error(`Server responded with invalid status: ${response.statusText}`);
    }
      ...
      <PageSection variant="light">
        <FormAccess isHorizontal onSubmit={handleSubmit(save)} role="manage-clients">
          <FormProvider {...form}>
            ...
            <ActionGroup>
              <FormSubmitButton formState={formState} allowInvalid allowNonDirty>
                {t("save")}
              </FormSubmitButton>
              <Button
                variant="link"
                component={props => <Link {...props} to={toClients({ realm })} />}
              >
                {t("cancel")}
              </Button>
            </ActionGroup>
