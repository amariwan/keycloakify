...
export const ClientScopes = ({
  ...
  const loader = async (first?: number, max?: number, search?: string) => {
    ...
    const find = (id: string) => clientScopes.find(clientScope => id === clientScope.id);
          ...
          onAdd={async scopes => {
            try {
              await Promise.all(
                scopes.map(
                  async scope =>
                    await addClientScope(adminClient, clientId, scope.scope, scope.type!)
                )
              );
              addAlert(t("clientScopeSuccess"), AlertVariant.success);
              refresh();
        ...
        columns={[
          {
            ...
            cellRenderer: row => {
              if (isDedicatedRow(row)) {
                return <Link to={toDedicatedScope({ realm, clientId })}>{row.name}</Link>;
              }
              return row.name!;
            }
          },
