...
const createEdge = (fromNode: string, toNode: string, label?: ConditionLabel): Edge => ({
  ...
  label: label,
  data: {
    onEdgeClick: (evt: ReactMouseEvent<HTMLButtonElement, MouseEvent>, id: string) => {
      evt.stopPropagation();
      alert(`hello ${id}`);
    }
  }
...
const renderSubFlow = (execution: ExpandableExecution): IntermediateFlowResult => {
  if (!execution.executionList) throw new Error("Execution list is required for subflow");
...
function renderGraph(executionList: ExpandableExecution[]): [Node[], Edge[]] {
  const executionListNoDisabled = executionList.filter(e => e.requirement !== "DISABLED");
  const groupings = [
