...
export function DraggableTable<T>({
  ...
  const onDragOver = (evt: ReactDragEvent) => {
    ...
    if (
      ...
      const draggingToItemIndex = Array.from(bodyRef.current?.children || []).findIndex(
        item => item.id === dragId
      );
      ...
      >
        {data.map(row => (
          ...
          >
            ...
            />
            {columns.map(column => (
              <Td key={`${get(row, keyField)}_${column.name}`} dataLabel={column.name}>
                {column.cellRenderer ? column.cellRenderer(row) : get(row, column.name)}
              </Td>
            ))}
            {actions && (
              <Td isActionCell>
                <ActionsColumn
                  items={actions.map(({ isActionable, ...action }) =>
                    isActionable ? { ...action, isDisabled: !isActionable(row) } : action
                  )}
                  rowData={row!}
                />
              </Td>
