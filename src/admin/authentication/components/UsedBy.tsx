...
const UsedByModal = ({ id, isSpecificClient, onClose }: UsedByModalProps) => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      actions={[
        <Button data-testid="cancel" id="modal-cancel" key="cancel" onClick={onClose}>
          {t("close")}
        </Button>
      ]}
    >
...
export const UsedBy = ({ authType: { id, usedBy } }: UsedByProps) => {
  ...
  return (
    <>
      ...
      {(usedBy?.type === "SPECIFIC_PROVIDERS" || usedBy?.type === "SPECIFIC_CLIENTS") &&
          ...
          <Popover
            ...
            bodyContent={
              <div key={`usedBy-${id}-${usedBy.values}`}>
                {t(
                  "appliedBy" +
                    (usedBy.type === "SPECIFIC_CLIENTS" ? "Clients" : "Providers")
                )}{" "}
      ...
      {usedBy?.type === "DEFAULT" && <Label label={t(`flow.${REALM_FLOWS.get(key!)}`)} />}
