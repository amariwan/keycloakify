  ...
  return (
    ...
    >
      <Form isHorizontal>
        ...
        <SwitchField field="config.backchannelSupported" label="backchannelLogout" />
        <SwitchField
          field="config.sendIdTokenOnLogout"
          label="sendIdTokenOnLogout"
          defaultValue={"true"}
        />
        <SwitchField field="config.sendClientIdOnLogout" label="sendClientIdOnLogout" />
            ...
            render={({ field }) => (
              ...
              >
                {Object.entries(promptOptions).map(([key, val]) => (
                  <SelectOption selected={val === field.value} key={key} value={val}>
                    {t(`prompts.${key}`)}
                  </SelectOption>
                ))}
              </KeycloakSelect>
