...
export default function AttributesGroupForm() {
  ...
  const [generatedAttributesGroupDisplayName, setGeneratedAttributesGroupDisplayName] =
    useState("");
  const [
      ...
      await Promise.all(
        combinedLocales.map(async (locale: string) => {
          try {
            ...
            const displayDescriptionKey = extractKey(formData.displayDescription) || "";

            const headerTranslation = translations[displayHeaderKey] || "";
            const descriptionTranslation = translations[displayDescriptionKey] || "";
        ...
        displayHeader: {
          key:
            translationsToSaveDisplayHeader.length > 0
              ...
              : "",
          translations: translationsToSaveDisplayHeader.flatMap(data => data.translations)
        },
        displayDescription: {
          key:
  ...
  const saveTranslations = async () => {
    ...
    try {
      if (translationsData && translationsData.displayHeader.translations.length > 0) {
  ...
  const formattedAttributesGroupDisplayName = attributesGroupDisplayName?.substring(
    2,
  ...
  return (
    <>
      {addTranslationsModalOpen && (
        <AddTranslationsDialog
          translationKey={
            type === "displayHeader" ? groupDisplayNameKey : groupDisplayDescriptionKey
          }
