...
export const AddTranslationsDialog = ({
  ...
  useEffect(() => {
    combinedLocales.forEach((locale, rowIndex) => {
      ...
      const translationExists = translations.translations[rowIndex] !== undefined;
      setValue(
  ...
  }, [combinedLocales, defaultTranslations, translationKey, setValue, translations]);
  ...
  const handleOk = () => {
    ...
    const updatedTranslations = formData.translations.map(translation => {
      if (translation.locale === filter) {
        return {
          ...translation,
          value: formData.translations.find(t => t.locale === filter)?.value ?? ""
        };
      }
      return translation;
    });
