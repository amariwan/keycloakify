  ...
  return (
    <FormAccess isHorizontal role="manage-realm" onSubmit={handleSubmit(save)}>
      <FormProvider {...form}>
        ...
        <DynamicComponents
          properties={
            allComponentTypes.find(type => type.id === providerType)?.properties || []
          }
        />
        <ActionGroup>
          <Button data-testid="add-provider-button" variant="primary" type="submit">
            {t("save")}
          </Button>
  ...
  return (
    <>
      ...
      <PageSection variant="light">
        <KeyProviderForm
          {...params}
          onClose={() => navigate(toKeysTab({ realm: params.realm, tab: "providers" }))}
        />
      </PageSection>
    </>
  );
