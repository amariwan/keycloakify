    ...
    try {
      ...
      const filteredMessages =
        filter.hasWords.length > 0
          ? messages.filter(m =>
              filter.hasWords.some(f => m.value.includes(f) || m.key.includes(f))
            )
          : messages;
        ...
        <FlexItem>
          ...
          >
            ...
            >
              ...
              <FormGroup label={t("themeType")} fieldId="kc-themeType" isRequired>
                <Controller
                  name="themeType"
                  control={control}
                  defaultValue=""
                      ...
                      <ChipGroup>
                        {field.value.map((word, index) => (
                          <Chip
                            ...
                            onClick={e => {
                              e.stopPropagation();
                              const newWords = field.value.filter((_, i) => i !== index);
                ...
                return (
                  ...
                  >
                    {typeof value === "string" ? (
                      <Chip isReadOnly>
                        {key === "locale"
                          ? localeToDisplayName(value, whoAmI.getLocale())?.toLowerCase()
                          : value}
                      </Chip>
                    ) : (
                      value.map(entry => (
