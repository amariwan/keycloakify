...
export const RealmOverrides = ({
    ...
    onConfirm: async () => {
      try {
        for (const key of selectedRowKeys) {
          delete (
            i18n.store.data[whoAmI.getLocale()][currentRealm] as Record<string, string>
          )[key];
  ...
  const toggleSelectAllRows = () => {
    if (areAllRowsSelected) {
      ...
      setSelectedRowKeys(tableRows.map(row => (row.cells?.[0] as IRowCell).props.value));
    }
    ...
    <>
              ...
              >
                <DropdownList>
                  <DropdownItem
                    ...
                    isDisabled={translations.length === 0 || selectedRowKeys.length === 0}
                    onClick={() => {
                      toggleDeleteDialog();
                      setKebabOpen(false);
                    }}
      ...
      >
        ...
        {translations.length !== 0 && (
          <Table aria-label={t("editableRowsTable")} data-testid="editable-rows-table">
            <Thead>
              <Tr>
                ...
                <Tr key={(row.cells?.[0] as IRowCell).props.value}>
                  <Td
                    ...
                    select={{
                      rowIndex,
                      onSelect: event =>
                        handleRowSelect(event as ChangeEvent<HTMLInputElement>, rowIndex),
                      isSelected: isRowSelected((row.cells?.[0] as IRowCell).props.value)
                    }}
                  />
                    ...
                    >
                      ...
                      >
                        {editStates[rowIndex] ? (
                          ...
                          <>
                            <span>{(row.cells?.[1] as IRowCell).props.value}</span>
                            <Button
                              onClick={() => {
                                const currentValue = (
