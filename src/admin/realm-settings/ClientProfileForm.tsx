...
export default function ClientProfileForm() {
  ...
  useFetch(
    ...
    profiles => {
      ...
      const globalProfile = profiles.globalProfiles?.find(p => p.name === profileName);
  ...
  const save = async (form: ClientProfileForm) => {
    ...
    try {
      ...
      addAlert(
        editMode ? t("updateClientProfileSuccess") : t("createClientProfileSuccess"),
        AlertVariant.success
      );

      navigate(toClientProfile({ realm, profileName: form.name }));
    } catch (error) {
      addError(editMode ? "updateClientProfileError" : "createClientProfileError", error);
    }
  };
        ...
        badges={[
          {
            ...
            text: isGlobalProfile ? <Label color="blue">{t("global")}</Label> : ""
          }
        ]}
        divider
        dropdownItems={
            ...
            {editMode && (
              <>
                      ...
                      {profileExecutors.map((executor, idx) => (
                        ...
                        >
                          <DataListItemRow data-testid="executors-list-row">
                            <DataListItemCells
                              dataListCells={[
                                <DataListCell key="executor" data-testid="executor-type">
                                  ...
                                  {executorTypes
                                    ?.filter(type => type.id === executor.executor)
                                    .map(type => (
                ...
                {profileExecutors.length === 0 && (
                  <>
                    <Divider />
                    <Text className="kc-emptyExecutors" component={TextVariants.h2}>
                      {t("emptyExecutors")}
                    </Text>
                  </>
                )}
