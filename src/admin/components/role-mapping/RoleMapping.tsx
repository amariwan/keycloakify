...
export const mapRoles = (assignedRoles: Row[], effectiveRoles: Row[], hide: boolean) => [
  ...(hide
    ...
    : effectiveRoles.map(row => ({
        ...
        role: {
          ...row.role,
          isInherited: assignedRoles.find(r => r.role.id === row.role.id) === undefined
        }
      })))
];
  ...
  const loader = async () => {
    ...
    if (!hide) {
      ...
      effectiveRoles = effectiveRoles.filter(
        role =>
          !effectiveClientRoles.some(clientRole => clientRole.role.id === role.role.id)
      );
    }
  ...
  return (
    <>
      ...
      <KeycloakDataTable
        ...
        isRowDisabled={value => (value.role as CompositeRole).isInherited || false}
        toolbarItem={
          <>
            <ToolbarItem>
              <Checkbox
