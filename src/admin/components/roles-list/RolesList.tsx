  ...
  return role.name !== defaultRoleName ? (
    ...
    <>
      {canViewUserRegistration ? (
        <Link to={toRealmSettings({ realm, tab: "user-registration" })}>{role.name}</Link>
      ) : (
        <span>{role.name}</span>
      )}
      <HelpItem helpText={t(`${messageBundle}:defaultRole`)} fieldLabelId="defaultRole" />
    </>
  );
};
      ...
      <KeycloakDataTable
            ...
            : [
                {
                  ...
                  onRowClick: role => {
                    ...
                    if (realm?.defaultRole && role.name === realm!.defaultRole!.name) {
        ...
        emptyState={
          <ListEmptyState
            ...
            instructions={isReadOnly ? "" : t(`noRolesInstructions-${messageBundle}`)}
