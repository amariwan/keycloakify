...
export const PermissionsTab = ({ id, type }: PermissionsTabProps) => {
  ...
  const togglePermissionEnabled = (enabled: boolean) => {
    switch (type) {
      case "clients":
        return adminClient.clients.updateFineGrainPermission({ id: id! }, { enabled });
      case "users":
          ...
          <Card isFlat className="keycloak__permission__permission-table">
            <CardBody className="pf-v5-u-p-0">
              <Table aria-label={t("permissionsList")} variant="compact">
                <Thead>
                  <Tr>
                    <Th id="permissionsScopeName">{t("permissionsScopeName")}</Th>
                ...
                <Tbody>
                  {localeSort(
                    ...
                    <Tr key={id}>
                      ...
                      <Td>{t(`scopePermissions.${type}.${name}-description`)}</Td>
                      <Td isActionCell>
                        <ActionsColumn
                          items={[
                            {
