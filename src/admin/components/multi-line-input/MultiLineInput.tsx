...
export const MultiLineInput = ({
  ...
  const fields = useMemo<string[]>(() => {
    let values = stringify
      ? stringToMultiline(Array.isArray(value) && value.length === 1 ? value[0] : value)
      : Array.isArray(value)
        ? value
        : [value];

    if (!Array.isArray(values) || values.length === 0) {
      values = (stringify ? stringToMultiline(defaultValue as string) : defaultValue) || [
        ""
      ];
    }
  ...
  useEffect(() => {
    register(name, {
      validate: value =>
        isRequired && toStringValue(value || []).length === 0 ? t("required") : undefined
    });
  }, [register]);
