  ...
  useFetch(
    () => {
      ...
      if (values?.length && !search) {
        return Promise.all(values.map((id: string) => adminClient.users.findOne({ id })));
      }
          ...
          <Select
            ...
            toggle={ref => (
              ...
              >
                <TextInputGroup isPlain>
                  ...
                  >
                    {variant === "typeaheadMulti" && Array.isArray(field.value) && (
                        ...
                        {field.value.map((selection: string, index: number) => (
                          <Chip
                            ...
                            onClick={ev => {
                              ev.stopPropagation();
                              field.onChange(
                                field.value.filter((item: string) => item !== selection)
                              );
                            }}
                          >
                            {users.find(u => u?.id === selection)?.username}
                          </Chip>
                        ))}
                      </ChipGroup>
                    )}
            ...
            onSelect={(_, v) => {
              ...
              if (variant !== "typeaheadMulti") {
                ...
                setInputValue(
                  removed ? "" : users.find(u => u?.id === option)?.username || ""
                );
                setOpen(false);
              } else {
                const changedValue = field.value.find((v: string) => v === option)
