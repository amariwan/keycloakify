  ...
  useEffect(() => {
    if (canSelectAll) {
      ...
      if (selectAllCheckbox) {
        ...
        checkbox.indeterminate = selected.length < rows.length && selected.length > 0;
      }
    }
  }, [selectedRows]);
    ...
    >
      <Thead>
        <Tr>
              ...
              select={
                !isRadio
                  ? {
                      ...
                      isSelected:
                        selectedRows.filter(r => r === true).length === rows.length
                    }
                  : undefined
              }
            />
          ...
          {columns.map(column => (
            <Th key={column.displayKey} className={column.transforms?.[0]().className}>
    ...
    return data
      .map((value, index) => {
        ...
        if (detailColumns) {
          row.push({
            parent: index * 2,
            cells: isDetailColumnsEnabled(value) ? renderCell(detailColumns!, value) : []
          } as SubRow<T>);
        }
        return row;
      })
      ...
      search === "" || isPaginated
        ...
        : convertToColumns(unPaginatedData || [])
            .filter(row =>
              row.cells.some(
                cell =>
                  cell && getNodeText(cell).toLowerCase().includes(search.toLowerCase())
              )
            )
